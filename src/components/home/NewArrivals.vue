      ...
      >
        ...
        <template v-if="loading">
          <article
            ...
            :key="'skel-'+n"
            class="snap-start w-72 flex-shrink-0 h-[440px]
        ...
        <template v-else>
          <article
            ...
            class="snap-start w-72 flex-shrink-0 h-[440px]
                   ...
                   duration-500 ease-in-out flex flex-col overflow-hidden"
          >
            ...
            <!-- Кнопки Купити / Повідомити -->
            <div class="px-4 pb-4">
              <button
                v-if="product.has_available_variant"
                @click="addToCart(product)"
                class="w-full h-11 bg-[#6B1F1F] hover:bg-[#861818] dark:bg-[#A01212] dark:hover:bg-[#c42e2e] text-white font-montserrat font-semibold rounded-lg flex items-center justify-between px-4 transition-all duration-300 ease-in-out shadow-sm hover:shadow-md"
              >
...
export default {
  ...
  methods: {
    ...
    async fetchProducts() {
  this.loading = true;      // ← почали лоадер
  try {
    const response = await api.getNewArrivals();
    const items = Array.isArray(response.data) ? response.data : response.data?.data || [];
    this.products = items
      .map(p => ({ ...p, has_available_variant: (p.variants||[]).some(v => v.is_available) }))
      .sort((a,b) => (b.has_available_variant?1:0)-(a.has_available_variant?1:0));
    this.updateProductsPerPage();
  } catch (error) {
    console.error('Помилка при завантаженні новинок:', error);
  } finally {
    this.loading = false;   // ← лоадер закінчився
  }
},
